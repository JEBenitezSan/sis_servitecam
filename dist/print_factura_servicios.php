...
$consul_num_cant = "SELECT COUNT(`servicios`.`observaciones`) AS `num_productos`
                FROM `detalle_factura` 
                    LEFT JOIN `servicios` ON `detalle_factura`.`id_servicio` = `servicios`.`id_servicio`
                    WHERE `detalle_factura`.`id_num_factura` = '$max_factura_numero'";
...
$pdf->SetFont('Arial', 'B', 8);
            ...
            $pdf->Cell (11, 6,'', 0, 0, 'C', 0);
			$pdf->Cell (14, 6, $row['prec_venta_detall'], 0, 0, 'C', 0);			
			$pdf->Cell (14, 6, $row['cant_detall'], 0, 0, 'C', 0);			
            $pdf->Cell (16, 6, $row['sub_total'], 0, 0, 'C', 0);			
            $pdf->Ln (6);
            $pdf->Cell(60, 3,'-------------------------------------------------------------------------', 0, 0, 'C', 0);
            $pdf->Ln ();
...
$pdf->Output('Fac_Servitecam_'.$cliente.'_Num_'.$max_factura_numero.'.pdf','i');
?>
